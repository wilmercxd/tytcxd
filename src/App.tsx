...
export default function App() {
  ...
  const handlePoliedroExtraction = () => {
    ...
    const data: PoliedroData = {
      ...
      antiguedad: extract(/Antiguedad Línea:\s*([^M\s]+)/i),
      comportamiento: extract(/Comportamiento Pago:\s*([^\n]+)/i),
      cartera: extract(/Estado Cartera:\s*([^\n]+)/i),
    };

    if (data.nombre === 'No encontrado' && data.cc === 'No encontrado') {
...
=========================================
  ...
  const filteredEquipment = useMemo(() => {
    return EQUIPMENT_DATA.filter(eq => {
      ...
      const matchesCategory = selectedCategory === 'TODOS' || eq.categoria === selectedCategory;
  ...
  const brands = useMemo(() => {
    const bnds = new Set(EQUIPMENT_DATA.map(e => e.marca));
  ...
  const deviceNames = useMemo(() => {
    const names = new Set(EQUIPMENT_DATA.filter(e => 
      (selectedBrand === 'TODOS' || e.marca === selectedBrand) && 
              ...
              <div className="space-y-8" id="step2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    <span className="font-bold text-xs uppercase tracking-widest text-slate-600">Celulares</span>
                  </button>
                  <button 
                    onClick={() => nextStep()}
                  ...
                  >
                    ...
                    <span className="font-bold text-xs uppercase tracking-widest text-slate-600">Portátiles</span>
                  </button>
                  <button 
                    onClick={() => nextStep()}
              ...
              <div className="space-y-6" id="step4">
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-tight">Equipos de Tecnología</label>
                      <select 
                        className="w-full p-2.5 border border-slate-200 rounded-lg text-xs bg-gray-50 font-bold text-slate-800"
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {[6, 12, 18, 24, 36].map(m => (
                        <button
                          key={m}
                          onClick={() => setTerm(m)}
                          className={`py-3 rounded-lg font-bold text-[10px] uppercase transition-all border ${
                ...
                {selectedObjection && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    ...
                    <div className="bg-slate-100 p-4 rounded-lg border-l-4 border-slate-400">
                      ...
                      <p className="text-xs leading-relaxed text-slate-600">"{OBJECTIONS[selectedObjection].responses.r2}"</p>
                    </div>
                  </motion.div>
                )}
              </div>
