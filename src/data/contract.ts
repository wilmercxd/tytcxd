export const generateContractText = (data: {
  nombre: string;
  cc: string;
  equipo: string;
  marca: string;
...
<div class="space-y-4 text-justify font-sans text-xs leading-relaxed">
  <p class="text-center font-bold text-lg mb-6 underline">CONTRATO DE FINANCIACIÓN DE EQUIPO DE TECNOLOGÍA O TERMINAL MÓVIL</p>
  ...
  <div class="border p-4 bg-gray-50 rounded">
    <p>Valor Total del Equipo: <strong>${data.precio}</strong> (IVA Incluido)</p>
    <p>IVA del Equipo: Incluido</p>
    <p>Pago Inicial: $ 0</p>
    <p>Saldo del Equipo: <strong>${data.precio}</strong></p>
    <p>Número de cuotas: <strong>${data.meses}</strong></p>
    <p>Valor de la Cuota: <strong>${data.cuota}</strong></p>
  ...
  <p class="mt-8 font-semibold">Recuerde que mi nombre es <strong>${data.asesor}</strong>, Bienvenido a Claro. Estamos comprometidos con el mejor servicio.</p>
</div>
`;
